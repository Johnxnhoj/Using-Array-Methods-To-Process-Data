...
var arr = items.filter(item => item.price > 14.0 && item.price < 18)
var titles = arr.map(title => title.title).join("/n")
console.log(arr)

document.querySelector("#answer2").innerHTML = `${titles}`

var code = items
  .filter(items => item.currency_code == "GBP")
  .map(name => `${name.title} cost &#163; ${name.price}`)

document.querySelector("#answer3").innerHTML = `${ccode}`
var w = items
  .filter(item => item.materials.includes("wood"))
  .map(name => `${name.title}`)
  .join("/n")
...
var sm = items
  .filter(item => item.who_made === "i_did")
...
document.querySelector(
  "#answer6"
).innerHTML = `${sm.length} were made by their sellers`
